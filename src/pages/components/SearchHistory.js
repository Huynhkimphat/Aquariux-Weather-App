import { useState, useEffect } from "react";

export default function SearchHistory(props) {
  const [searchHistory, setSearchHistory] = useState([]);

  useEffect(() => {
    const sortData = props.data.sort((a, b) => {
      const dateA = new Date(a.data_time);
      const dateB = new Date(b.data_time);
      return dateB.getTime() - dateA.getTime();
    });
    setSearchHistory(sortData);
  }, [props.data]);

  const handleRemoveSearchHistoryItem = (index) => {
    const newArray = [...searchHistory];
    newArray.splice(index, 1);
    props.removeSearchHistory(newArray.reverse());
  };

  return (
    <div className="w-[100%] dark:bg-violet-900 bg-violet-200 h-[100vh] dark:text-white text-black overflow-scroll p-4 md:p-8 rounded-xl no-scrollbar">
      <p className="text-xl pb-4">Search History</p>
      <div className="flex justify-between flex-col gap-y-4">
        {searchHistory &&
          searchHistory.map((item, index) => (
            <div
              key={index}
              className="w-[100%] dark:bg-violet-950 bg-violet-100 rounded-xl p-2 md:p-4 flex justify-between items-center"
            >
              <div>
                <div>
                  {item.name}, {item.country}
                </div>
                <div className="block lg:hidden text-gray-500 text-xs">
                  {item.data_time}
                </div>
              </div>
              <div className="text-gray-500 flex gap-x-4">
                <div className="hidden lg:block">{item.data_time}</div>
                <div
                  className="hover:text-gray-300"
                  onClick={() => props.updateSearchHistoryKey(item.name)}
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                    viewBox="0 0 24 24"
                    strokeWidth="1.5"
                    stroke="currentColor"
                    className="size-6"
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z"
                    />
                  </svg>
                </div>
                <div
                  className="hover:text-gray-300"
                  onClick={() => handleRemoveSearchHistoryItem(index)}
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                    viewBox="0 0 24 24"
                    strokeWidth="1.5"
                    stroke="currentColor"
                    className="size-6"
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0"
                    />
                  </svg>
                </div>
              </div>
            </div>
          ))}
      </div>
    </div>
  );
}
